...
bool MetalPlatform::isPlatformSupported() {
    // Check that we're running on macOS with a Metal-capable GPU.
    ...
    if (major < 14 || (major == 14 && minor < 3))
        ...
        // contained a number of serious bugs in the Apple GPU libraries.
        return false;
#endif

    // Make sure at least one Metal device is available.
    @autoreleasepool {
...
MetalPlatform::PlatformData::PlatformData(const System& system, const string& platformPropValue, const string& deviceIndexProperty,
    ...
    // Query device names using the Metal API.
    @autoreleasepool {
        ...
        for (int i = 0; i < (int) contexts.size(); i++) {
            ...
            id<MTLDevice> device = (__bridge id<MTLDevice>)contexts[i]->getMTLDevice();
            deviceName << string([[device name] UTF8String]);
        }
        ...
        // For Metal, the platform name is always "Apple Metal".
        propertyValues[MetalPlatform::MetalPlatformName()] = "Apple Metal";
