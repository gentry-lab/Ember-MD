 ...
 */
KERNEL void sortShortList(GLOBAL DATA_TYPE* restrict data, uint length, LOCAL DATA_TYPE* dataBuffer,
        MM_THREAD_ARGS) {
 ...
 */
KERNEL void sortShortList2(GLOBAL const DATA_TYPE* restrict dataIn, GLOBAL DATA_TYPE* restrict dataOut, int length,
        MM_THREAD_ARGS) {
 ...
 */
KERNEL void computeRange(GLOBAL const DATA_TYPE* restrict data, uint length, GLOBAL KEY_TYPE* restrict range, LOCAL KEY_TYPE* restrict minBuffer,
        LOCAL KEY_TYPE* restrict maxBuffer, uint numBuckets, GLOBAL uint* restrict bucketOffset,
        MM_THREAD_ARGS) {
#if UNIFORM
 ...
 */
KERNEL void assignElementsToBuckets(GLOBAL const DATA_TYPE* restrict data, uint length, uint numBuckets, GLOBAL const KEY_TYPE* restrict range,
        GLOBAL uint* restrict bucketOffset, GLOBAL uint* restrict bucketOfElement, GLOBAL uint* restrict offsetInBucket,
        MM_THREAD_ARGS) {
    ...
    if (get_global_id(0) == get_local_id(0)+1)
        ((GLOBAL char*)bucketOffset)[sizeof(int)*numBuckets+1] = 0;
#endif
 ...
 */
KERNEL void assignElementsToBuckets2(GLOBAL const DATA_TYPE* restrict data, uint length, uint numBuckets, GLOBAL const KEY_TYPE* restrict range,
        GLOBAL uint* restrict bucketOffset, GLOBAL uint* restrict bucketOfElement, GLOBAL uint* restrict offsetInBucket,
        MM_THREAD_ARGS) {
 ...
 * is executed as a single work group.
 */
KERNEL void computeBucketPositions(uint numBuckets, GLOBAL uint* restrict bucketOffset, LOCAL uint* restrict buffer,
        MM_THREAD_ARGS) {
    uint globalOffset = 0;
 ...
 */
KERNEL void copyDataToBuckets(GLOBAL const DATA_TYPE* restrict data, GLOBAL DATA_TYPE* restrict buckets, uint length, GLOBAL const uint* restrict bucketOffset, GLOBAL const uint* restrict bucketOfElement, GLOBAL const uint* restrict offsetInBucket,
        MM_THREAD_ARGS) {
    for (uint index = get_global_id(0); index < length; index += get_global_size(0)) {
 ...
 */
KERNEL void sortBuckets(GLOBAL DATA_TYPE* restrict data, GLOBAL const DATA_TYPE* restrict buckets, uint numBuckets, GLOBAL const uint* restrict bucketOffset, LOCAL DATA_TYPE* restrict buffer,
        MM_THREAD_ARGS) {
