...
KERNEL void computeNonbonded(
        GLOBAL float* restrict forceBuffers,
        GLOBAL mixed* restrict energyBuffer, GLOBAL const real4* restrict posq, GLOBAL const unsigned int* restrict exclusions,
        GLOBAL const int2* restrict exclusionTiles, unsigned int startTileIndex, unsigned long numTileIndices
#ifdef USE_CUTOFF
    ...
    mixed energy = 0;
    INIT_DERIVATIVES
    LOCAL AtomData localData[FORCE_WORK_GROUP_SIZE];
    ...
    for (int pos = firstExclusionTile; pos < lastExclusionTile; pos++) {
        ...
        ATOMIC_ADD(&forceBuffers[offset], force.x);
        ATOMIC_ADD(&forceBuffers[offset+PADDED_NUM_ATOMS], force.y);
        ATOMIC_ADD(&forceBuffers[offset+2*PADDED_NUM_ATOMS], force.z);
        if (x != y) {
            offset = y*TILE_SIZE + tgx;
            ATOMIC_ADD(&forceBuffers[offset], localData[get_local_id(0)].fx);
            ATOMIC_ADD(&forceBuffers[offset+PADDED_NUM_ATOMS], localData[get_local_id(0)].fy);
            ATOMIC_ADD(&forceBuffers[offset+2*PADDED_NUM_ATOMS], localData[get_local_id(0)].fz);
        }
#endif
    }
...
#endif
            ATOMIC_ADD(&forceBuffers[atom1], force.x);
            ATOMIC_ADD(&forceBuffers[atom1+PADDED_NUM_ATOMS], force.y);
            ATOMIC_ADD(&forceBuffers[atom1+2*PADDED_NUM_ATOMS], force.z);
            if (atom2 < PADDED_NUM_ATOMS) {
                ATOMIC_ADD(&forceBuffers[atom2], localData[get_local_id(0)].fx);
                ATOMIC_ADD(&forceBuffers[atom2+PADDED_NUM_ATOMS], localData[get_local_id(0)].fy);
                ATOMIC_ADD(&forceBuffers[atom2+2*PADDED_NUM_ATOMS], localData[get_local_id(0)].fz);
            }
#endif
        }
        pos++;
