 ...
 */
KERNEL void packForwardData(GLOBAL const real* restrict in, GLOBAL real2* restrict out,
        MM_THREAD_ARGS) {
 ...
 */
KERNEL void unpackForwardData(GLOBAL const real2* restrict in, GLOBAL real2* restrict out, LOCAL real2* restrict w,
        MM_THREAD_ARGS) {
 ...
 */
real2 loadComplexValue(GLOBAL const real2* restrict in, int x, int y, int z) {
 ...
 */
KERNEL void packBackwardData(GLOBAL const real2* restrict in, GLOBAL real2* restrict out, LOCAL real2* restrict w,
        MM_THREAD_ARGS) {
    // Compute the phase factors.
 ...
 */
KERNEL void unpackBackwardData(GLOBAL const real2* restrict in, GLOBAL real* restrict out,
        MM_THREAD_ARGS) {
    const int gridSize = PACKED_XSIZE*PACKED_YSIZE*PACKED_ZSIZE;
    for (int index = get_global_id(0); index < gridSize; index += get_global_size(0)) {
        int x = index/(PACKED_YSIZE*PACKED_ZSIZE);
