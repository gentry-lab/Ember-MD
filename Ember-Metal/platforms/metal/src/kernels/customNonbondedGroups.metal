 ...
 * every thread.
 */
DEVICE int reduceMax(int val, LOCAL_ARG int* temp, MM_THREAD_PARAMS) {
...
KERNEL void computeInteractionGroups(
    ...
    for (int tile = startTile; tile < endTile; tile++) {
        ...
        int tj = tgx;
        int rangeStop = rangeStart + reduceMax(rangeEnd-rangeStart, reductionBuffer, MM_THREAD_PASS);
        SYNC_WARPS;
...
KERNEL void buildNeighborList(GLOBAL int* RESTRICT rebuildNeighborList, GLOBAL int* RESTRICT numGroupTiles,
    ...
    LOCAL bool anyInteraction[WARPS_IN_BLOCK];
    LOCAL int tileIndex[WARPS_IN_BLOCK];
    LOCAL int reductionBuffer[LOCAL_MEMORY_SIZE];
