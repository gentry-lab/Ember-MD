...
void MetalBondedUtilities::initialize(const System& system) {
    ...
    for (int i = 0; i < numForces; i++) {
        ...
        atomIndices[i].initialize<unsigned int>(context, indexVec.size(), "bondedIndices");
    ...
    try {
        program = context.compileProgram(s.str(), defines);
    } catch (OpenMMException& e) {
        fprintf(stderr, "[Metal] Bonded kernel compilation failed (%d force sources, %d energy derivs)\n",
                (int)forceSource.size(), (int)energyParameterDerivatives.size());
        throw;
    }
...
void MetalBondedUtilities::computeInteractions(int groups) {
    ...
    if (!hasInitializedKernels) {
        ...
        kernel->addArg((int) groups);  // placeholder for groups
        if (context.getUseDoublePrecision()) {
            kernel->addArg(context.getPeriodicBoxSizeDouble());
            kernel->addArg(context.getInvPeriodicBoxSizeDouble());
            kernel->addArg(context.getPeriodicBoxVecXDouble());
            kernel->addArg(context.getPeriodicBoxVecYDouble());
            kernel->addArg(context.getPeriodicBoxVecZDouble());
        }
        else {
